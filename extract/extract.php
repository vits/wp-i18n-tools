	...
	function extract_from_directory( $dir, $excludes = array(), $includes = array(), $prefix = '' ) {
		...
		foreach ( $file_names as $file_name ) {
			...
			if ( preg_match( '/\.(?:php|js)$/', $file_name ) && $this->does_file_name_match( $prefix . $file_name, $excludes, $includes ) ) {
				$tmp = $this->extract_from_file( $file_name, $prefix );
				$translations->merge_originals_with( $tmp );
			}
			if ( is_dir( $file_name ) ) {
				$tmp = $this->extract_from_directory( $file_name, $excludes, $includes, $prefix . $file_name . '/' );
				$translations->merge_originals_with( $tmp );
			}
		}
		chdir( $old_cwd );
